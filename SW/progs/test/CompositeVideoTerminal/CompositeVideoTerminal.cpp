...
ISR(TIMER3_OVF_vect) { // TIMER1_OVF vector occurs at the start of each scan line's sync pulse
	if (++scanline == 312) {
		OCR3B = 948; // scan lines 0 - 7 have wide 59.3us sync pulses
		scanline = 0;
		} else if (scanline == 8) {
		OCR3B = 74; // swap to short 4.7us sync pulses for scan lines 8 - 311
...
ISR(TIMER3_COMPA_vect) { // occurs at start of 'text safe' area of scan lines 51 - 280
	static uint8_t *pScreenRam;
	static const uint8_t *fontSlice;
	static uint8_t slice;
	
	uint16_t tcnt = TCNT3; // capture timer to allow jitter correction
	...
	if (scanline == TOP_EDGE) { // on stored-up 'false trigger' scanline, initialize the pointers
		...
		if (scanline == BOTTOM_EDGE) {
			TIMSK3 &= ~_BV(OCIE3A); // we don't want any more COMPB interrupts this frame
...
void setup() {
  for (int i = 8 * BYTES_PER_RASTER; i < 10 * BYTES_PER_RASTER; ++i) {
    screenRam[i] |= 0x80;  // make the characters on rows 8 and 9 RVS field
  	...
  	Cset_output(RCA_SIGNAL);
	Cset_output(RCA_SUPPRESS);
...
//	pinMode(PIN_B, INPUT_PULLUP);
	...
	UBRR1L = UBRR1H = 0x00; // fastest possible baud
	...
	// configure timer/counter 1 to output scanline sync pulses on Pin9 (OC1A)		# OC3B
	// use mode 7 (fast PWM 10-bit count to TOP=1023) at 16MHz fclk - one cycle per 64us scanline
	cli(); // not necessary
	TCCR3A =  _BV(COM3B1) | _BV(COM3B0) | _BV(WGM11) | _BV(WGM10); // set OC1A output on compare match, (mode 3 so far)
	TCCR3B = _BV(WGM32) | _BV(CS30); // now mode 7 at clk/1 (16MHz)
	OCR3B = 948; // 59.3us wide sync pulse for first 8 scan lines
	OCR3A = LEFT_EDGE;
	TIMSK3 = _BV(TOIE3); // _BV(OCIE1A);
	TCNT3 = 0x0000;
	sei(); // necessary
